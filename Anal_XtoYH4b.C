...
float* TriggerSF_jets(TFile *file_trigger_jets[ntrigger_etas][ntrigger_jets], vector<AK4Jet> inputjets){
	...
	for(int ijet=0; ijet<max(int(jets.size()),4); ijet++){
		...
		TGraphErrors *grjet = (TGraphErrors*)file_trigger_jets[ieta_indx][ijet]->Get(histname);
...
float* TriggerSF_btag(TFile *file_trigger_btag, vector<AK4Jet> inputjets, string year){
	...
	double x_input = (jets[0].btag_PNetB+jets[1].btag_PNetB);	
	const double* xArray = grjet->GetX();
...
float* TriggerSF_L1HT(TFile *file_trigger_L1HT, vector<AK4Jet> inputjets, string year){
	...
	string histname;
	if(year=="2022"||year=="2022EE") {  histname = "l1_2022postEE_HT"; }
...
//   fileout->SetCompressionLevel(4); 
   ...
   Tout->Branch("Flag_PNet_isSR_L", &Flag_PNet_isSR_L, "Flag_PNet_isSR_L/O");
   Tout->Branch("Flag_PNet_isSR_M", &Flag_PNet_isSR_M, "Flag_PNet_isSR_M/O");
   Tout->Branch("Flag_PNet_isSR_T", &Flag_PNet_isSR_T, "Flag_PNet_isSR_T/O");
   Tout->Branch("Flag_PNet_isSR_XT", &Flag_PNet_isSR_XT, "Flag_PNet_isSR_XT/O");
   Tout->Branch("Flag_PNet_isSR_XXT", &Flag_PNet_isSR_XXT, "Flag_PNet_isSR_XXT/O");
   
   Tout->Branch("Flag_PNet_isVR_L", &Flag_PNet_isVR_L, "Flag_PNet_isVR_L/O");
   Tout->Branch("Flag_PNet_isVR_M", &Flag_PNet_isVR_M, "Flag_PNet_isVR_M/O");
   Tout->Branch("Flag_PNet_isVR_T", &Flag_PNet_isVR_T, "Flag_PNet_isVR_T/O");
   Tout->Branch("Flag_PNet_isVR_XT", &Flag_PNet_isVR_XT, "Flag_PNet_isVR_XT/O");
   Tout->Branch("Flag_PNet_isVR_XXT", &Flag_PNet_isVR_XXT, "Flag_PNet_isVR_XXT/O");
   
   Tout->Branch("Flag_PNet_isCR_L", &Flag_PNet_isCR_L, "Flag_PNet_isCR_L/O");
   Tout->Branch("Flag_PNet_isCR_M", &Flag_PNet_isCR_M, "Flag_PNet_isCR_M/O");
   Tout->Branch("Flag_PNet_isCR_T", &Flag_PNet_isCR_T, "Flag_PNet_isCR_T/O");
   Tout->Branch("Flag_PNet_isCR_XT", &Flag_PNet_isCR_XT, "Flag_PNet_isCR_XT/O");
   Tout->Branch("Flag_PNet_isCR_XXT", &Flag_PNet_isCR_XXT, "Flag_PNet_isCR_XXT/O");
   
   Tout->Branch("Flag_ParT_isSR_L", &Flag_ParT_isSR_L, "Flag_ParT_isSR_L/O");
   Tout->Branch("Flag_ParT_isSR_M", &Flag_ParT_isSR_M, "Flag_ParT_isSR_M/O");
   Tout->Branch("Flag_ParT_isSR_T", &Flag_ParT_isSR_T, "Flag_ParT_isSR_T/O");
   Tout->Branch("Flag_ParT_isSR_XT", &Flag_ParT_isSR_XT, "Flag_ParT_isSR_XT/O");
   Tout->Branch("Flag_ParT_isSR_XXT", &Flag_ParT_isSR_XXT, "Flag_ParT_isSR_XXT/O");
   
   Tout->Branch("Flag_ParT_isVR_L", &Flag_ParT_isVR_L, "Flag_ParT_isVR_L/O");
   Tout->Branch("Flag_ParT_isVR_M", &Flag_ParT_isVR_M, "Flag_ParT_isVR_M/O");
   Tout->Branch("Flag_ParT_isVR_T", &Flag_ParT_isVR_T, "Flag_ParT_isVR_T/O");
   Tout->Branch("Flag_ParT_isVR_XT", &Flag_ParT_isVR_XT, "Flag_ParT_isVR_XT/O");
   Tout->Branch("Flag_ParT_isVR_XXT", &Flag_ParT_isVR_XXT, "Flag_ParT_isVR_XXT/O");
   
   Tout->Branch("Flag_ParT_isCR_L", &Flag_ParT_isCR_L, "Flag_ParT_isCR_L/O");
   Tout->Branch("Flag_ParT_isCR_M", &Flag_ParT_isCR_M, "Flag_ParT_isCR_M/O");
   Tout->Branch("Flag_ParT_isCR_T", &Flag_ParT_isCR_T, "Flag_ParT_isCR_T/O");
   Tout->Branch("Flag_ParT_isCR_XT", &Flag_ParT_isCR_XT, "Flag_ParT_isCR_XT/O");
   Tout->Branch("Flag_ParT_isCR_XXT", &Flag_ParT_isCR_XXT, "Flag_ParT_isCR_XXT/O");
   ...
   for (int ij=0; ij<nentries; ij++) {
    ...
    for(auto & jet: Jets){
		...
		if(jet.applyReg){
			...
			float JEC_L2 = read_jec_single_fromCorrectiolib(cset_jec,jetInfo_jec,jec_tag,"L2Relative","AK4PFPuppiPNetRegressionPlusNeutrino");
		
			jet.JEC_bReg = JEC_L2;
		
			if(!isMC) {
				jetInfo_jec["JetPt"] = (jet_raw_p4.Pt()*jet.RegCorr*JEC_L2);
				jet.JEC_bReg  *= read_jec_single_fromCorrectiolib(cset_jec,jetInfo_jec,jec_tag,"L2L3Residual","AK4PFPuppiPNetRegressionPlusNeutrino");			
			}
    ...
    //cout<<"muons "<<vmuons.size()<<" AK4jets "<<Jets.size()<<endl;
    ...
    for(auto & jet: Jets){
		...
		if(isMC){
			...
			jet.btag_PNetB_SF = read_btagSF_fromCorrectiolib(cset_btag, "particleNet_shape",jetInfo_pnet);
			...
			jet.btag_RobustParTAK4B_SF = read_btagSF_fromCorrectiolib(cset_btag, "robustParticleTransformer_shape",jetInfo_parT);
			...
			for(int ibsys=0; ibsys<btag_systematics.size(); ibsys++){
				...
				jet.btag_PNetB_SF_up.push_back((abs(jet.hadronFlavour)!=4)?read_btagSF_fromCorrectiolib(cset_btag, "particleNet_shape",jetInfo_pnet):1.);
				
				jetInfo_pnet["systematic"] = "down_"+btag_systematics[ibsys];
				jet.btag_PNetB_SF_dn.push_back((abs(jet.hadronFlavour)!=4)?read_btagSF_fromCorrectiolib(cset_btag, "particleNet_shape",jetInfo_pnet):1.);
				
				// ParT //
				
				jetInfo_parT["systematic"] = "up_"+btag_systematics[ibsys];
				jet.btag_RobustParTAK4B_SF_up.push_back((abs(jet.hadronFlavour)!=4)?read_btagSF_fromCorrectiolib(cset_btag, "robustParticleTransformer_shape",jetInfo_parT):1.);
				
				jetInfo_parT["systematic"] = "down_"+btag_systematics[ibsys];
				jet.btag_RobustParTAK4B_SF_dn.push_back((abs(jet.hadronFlavour)!=4)?read_btagSF_fromCorrectiolib(cset_btag, "robustParticleTransformer_shape",jetInfo_parT):1.);
			...
			jet.btag_PNetB_SF = 1;  jet.btag_RobustParTAK4B_SF = 1;
			for(int ibsys=0; ibsys<btag_systematics.size(); ibsys++){
				jet.btag_PNetB_SF_up.push_back(0);  jet.btag_RobustParTAK4B_SF_up.push_back(0);
				jet.btag_PNetB_SF_dn.push_back(0);  jet.btag_RobustParTAK4B_SF_dn.push_back(0);
			}
	...
	*/
     ...
     */
     ...
     if(year=="2022"||year=="2022EE") { 
		 ...
		 if(hlt_PFHT280_QuadPFJet30_PNet2BTagMean0p55) { trig_threshold_pass = (Jets.size()>=4)?(Jets[3].pt>30. && HT_quadjet>280.):0; }
	}
     
	 // sorting jets by PNet b tag score //
	 std::sort(Jets.begin(), Jets.end(), [](const AK4Jet& a, const AK4Jet& b) { return a.btag_PNetB > b.btag_PNetB; });
     
     float btagsum_PNet = 0;
     if(Jets.size()>=2){ btagsum_PNet = Jets[0].btag_PNetB + Jets[1].btag_PNetB; }
     
     if(year=="2022"||year=="2022EE") { trig_threshold_pass = (trig_threshold_pass && (btagsum_PNet>0.65)); }
     else {
		if(hlt_QuadPFJet70_50_40_35_PNet2BTagMean0p65) { 	trig_threshold_pass = (trig_threshold_pass && (btagsum_PNet>0.65)); }
		if(hlt_PFHT280_QuadPFJet30_PNet2BTagMean0p55)  { 	trig_threshold_pass = (trig_threshold_pass && (btagsum_PNet>0.55)); }
	 }
    ...
    btag_PNet_weight = 1; btag_ParT_weight = 1;
    for(int ijet=0; ijet<Jets.size(); ijet++){
    ...
    if(isMC){    
	    ...
	    */
	    ...
	    if(Jets.size()>=4)
	    {
			...
			sfvalues_trig_jet = TriggerSF_jets(file_trigger_SF_jet,Jets);
   ...
   */ 
	...
	if(!Flag_pass_baseline){ 
		...
		for(int ijet=0; ijet<Jets.size(); ijet++){
			if(Jets[ijet].pt>30.){
				HT_jets += Jets[ijet].pt;
			}
		}
	 
		ST =  HT_jets + MET_pt;
	
		int nbjets_PNet_L = 0;	int nbjets_PartT_L = 0;
		int nbjets_PNet_M = 0;	int nbjets_PartT_M = 0;
		int nbjets_PNet_T = 0;	int nbjets_PartT_T = 0;
		int nbjets_PNet_XT = 0;	int nbjets_PartT_XT = 0;
		int nbjets_PNet_XXT = 0;	int nbjets_PartT_XXT = 0;
		
		for(int ijet=0; ijet<_s_nJetAK4; ijet++){
			if(_s_JetAK4_btag_PNetB_WP[ijet]>=5) { nbjets_PNet_XXT++; }
			if(_s_JetAK4_btag_PNetB_WP[ijet]>=4) { nbjets_PNet_XT++; }
			if(_s_JetAK4_btag_PNetB_WP[ijet]>=3) { nbjets_PNet_T++; }
			if(_s_JetAK4_btag_PNetB_WP[ijet]>=2) { nbjets_PNet_M++; }
			if(_s_JetAK4_btag_PNetB_WP[ijet]>=1) { nbjets_PNet_L++; }
		}		
		
		for(int ijet=0; ijet<_s_nJetAK4; ijet++){
			if(_s_JetAK4_btag_RobustParTAK4B_WP[ijet]>=5) { nbjets_PartT_XXT++; }
			if(_s_JetAK4_btag_RobustParTAK4B_WP[ijet]>=4) { nbjets_PartT_XT++; }
			if(_s_JetAK4_btag_RobustParTAK4B_WP[ijet]>=3) { nbjets_PartT_T++; }
			if(_s_JetAK4_btag_RobustParTAK4B_WP[ijet]>=2) { nbjets_PartT_M++; }
			if(_s_JetAK4_btag_RobustParTAK4B_WP[ijet]>=1) { nbjets_PartT_L++; }
		}
			
		Flag_PNet_isSR_L = Flag_PNet_isSR_M = Flag_PNet_isSR_T = Flag_PNet_isSR_XT = Flag_PNet_isSR_XXT = false;
		Flag_ParT_isSR_L = Flag_ParT_isSR_M = Flag_ParT_isSR_T = Flag_ParT_isSR_XT = Flag_ParT_isSR_XXT = false;
		Flag_PNet_isCR_L = Flag_PNet_isCR_M = Flag_PNet_isCR_T = Flag_PNet_isCR_XT = Flag_PNet_isCR_XXT = false;
		Flag_ParT_isCR_L = Flag_ParT_isCR_M = Flag_ParT_isCR_T = Flag_ParT_isCR_XT = Flag_ParT_isCR_XXT = false;
		Flag_PNet_isVR_L = Flag_PNet_isVR_M = Flag_PNet_isVR_T = Flag_PNet_isVR_XT = Flag_PNet_isVR_XXT = false;
		Flag_ParT_isVR_L = Flag_ParT_isVR_M = Flag_ParT_isVR_T = Flag_ParT_isVR_XT = Flag_ParT_isVR_XXT = false;
		
		Flag_PNet_isSR_L = (nbjets_PNet_L>=4)?true:false;  	 Flag_PNet_isVR_L = (nbjets_PNet_L==3)?true:false;   	  Flag_PNet_isCR_L = (nbjets_PNet_L==2)?true:false;
		Flag_PNet_isSR_M = (nbjets_PNet_M>=4)?true:false;  	 Flag_PNet_isVR_M = (nbjets_PNet_M==3)?true:false;   	  Flag_PNet_isCR_M = (nbjets_PNet_M==2)?true:false;
		Flag_PNet_isSR_T = (nbjets_PNet_T>=4)?true:false;  	 Flag_PNet_isVR_T = (nbjets_PNet_T==3)?true:false;   	  Flag_PNet_isCR_T = (nbjets_PNet_T==2)?true:false;
		Flag_PNet_isSR_XT = (nbjets_PNet_XT>=4)?true:false; 	 Flag_PNet_isVR_XT = (nbjets_PNet_XT==3)?true:false; 	  Flag_PNet_isCR_XT = (nbjets_PNet_XT==2)?true:false;
		Flag_PNet_isSR_XXT = (nbjets_PNet_XXT>=4)?true:false;  Flag_PNet_isVR_XXT = (nbjets_PNet_XXT==3)?true:false;  Flag_PNet_isCR_XXT = (nbjets_PNet_XXT==2)?true:false;
		
		Flag_ParT_isSR_L = (nbjets_PartT_L>=4)?true:false;  	  Flag_ParT_isVR_L = (nbjets_PartT_L==3)?true:false;   	 Flag_ParT_isCR_L = (nbjets_PartT_L==2)?true:false;
		Flag_ParT_isSR_M = (nbjets_PartT_M>=4)?true:false;      Flag_ParT_isVR_M = (nbjets_PartT_M==3)?true:false;   	 Flag_ParT_isCR_M = (nbjets_PartT_M==2)?true:false;
		Flag_ParT_isSR_T = (nbjets_PartT_T>=4)?true:false;  	  Flag_ParT_isVR_T = (nbjets_PartT_T==3)?true:false;   	 Flag_ParT_isCR_T = (nbjets_PartT_T==2)?true:false;
		Flag_ParT_isSR_XT = (nbjets_PartT_XT>=4)?true:false; 	  Flag_ParT_isVR_XT = (nbjets_PartT_XT==3)?true:false; 	 Flag_ParT_isCR_XT = (nbjets_PartT_XT==2)?true:false;
		Flag_ParT_isSR_XXT = (nbjets_PartT_XXT>=4)?true:false;  Flag_ParT_isVR_XXT = (nbjets_PartT_XXT==3)?true:false;   Flag_ParT_isCR_XXT = (nbjets_PartT_XXT==2)?true:false;
