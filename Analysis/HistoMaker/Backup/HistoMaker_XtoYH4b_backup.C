...
int main(int argc, char *argv[])
{
 ...
 float mass_frac = Mass_Fraction(MX_value,MY_value,125.);
 ...
 for(int ireg=0; ireg<nregion; ireg++){
	 ...
	 sprintf(histname_reg,"h_DNN_score_%s",region_names[ireg].c_str());
	 h_reg_dnn.push_back(getHisto1F(histname_reg, histname_reg, 50, 0, 1.));
 ...
 for(Long64_t jentry =0; jentry < tree_prescl->GetEntries() ; jentry++)
 {
   ...
   */ 
   ...
   if(jet_idx_btag.size()>=4){
	   ...
	   int Hcand_1_b_1_p4_idx[3], Hcand_1_b_2_p4_idx[3],  Hcand_2_b_1_p4_idx[3], Hcand_2_b_2_p4_idx[3];
	   bool H_1_truth[3] = {0};
	   bool H_2_truth[3] = {0};
	   bool b_matching = false;
	  
	   // check if all jets are matched to b quarks //
	   ...
	   // calling the H cand closest to H mass Hcand_1 //
...
//	   if(isSignal){
	   ...
	   for(int icomb=0; icomb<ncomb; icomb++){
		...
		bool b_pass = true;
		if(isSignal && !b_matching) { b_pass = false; }
		...
		for(int icomb=0; icomb<ncomb; icomb++){
			...
			//cout<<"Comb "<<icomb+1<<" score_pairing "<<score_pairing<<endl;
...
//	   }//isSignal
	   ...
	   for(int icomb=0; icomb<ncomb; icomb++){
		...
		h_jj_leading[ndijetvars*icomb+0]->Fill(Hcand_1[icomb].M(),weight_nom);  
		h_jj_leading[ndijetvars*icomb+1]->Fill(Hcand_1[icomb].Pt(),weight_nom); 
		h_jj_leading[ndijetvars*icomb+2]->Fill(min(jet_p4s[Hcand_1_b_1_p4_idx[icomb]].Pt(),jet_p4s[Hcand_1_b_2_p4_idx[icomb]].Pt())/max(jet_p4s[Hcand_1_b_1_p4_idx[icomb]].Pt(),jet_p4s[Hcand_1_b_2_p4_idx[icomb]].Pt()),weight_nom); 
		h_jj_leading[ndijetvars*icomb+3]->Fill(jet_p4s[Hcand_1_b_1_p4_idx[icomb]].DeltaR(jet_p4s[Hcand_1_b_2_p4_idx[icomb]]),weight_nom); 
		
		h_jj_subleading[ndijetvars*icomb+0]->Fill(Hcand_2[icomb].M(),weight_nom); 
		h_jj_subleading[ndijetvars*icomb+1]->Fill(Hcand_2[icomb].Pt(),weight_nom); 
		h_jj_subleading[ndijetvars*icomb+2]->Fill(min(jet_p4s[Hcand_2_b_1_p4_idx[icomb]].Pt(),jet_p4s[Hcand_2_b_2_p4_idx[icomb]].Pt())/max(jet_p4s[Hcand_2_b_1_p4_idx[icomb]].Pt(),jet_p4s[Hcand_2_b_2_p4_idx[icomb]].Pt()),weight_nom); 
		h_jj_subleading[ndijetvars*icomb+3]->Fill(jet_p4s[Hcand_2_b_1_p4_idx[icomb]].DeltaR(jet_p4s[Hcand_2_b_2_p4_idx[icomb]]),weight_nom); 
		
		h_quadjet_histograms[nquadjetvars*icomb+0]->Fill(Hcand_1[icomb].Eta()-Hcand_2[icomb].Eta(),weight_nom);
		h_quadjet_histograms[nquadjetvars*icomb+1]->Fill(Hcand_1[icomb].Rapidity()-Hcand_2[icomb].Rapidity(),weight_nom);
		h_quadjet_histograms[nquadjetvars*icomb+2]->Fill(PhiInRange(Hcand_1[icomb].Phi()-Hcand_2[icomb].Phi()),weight_nom);
		h_quadjet_histograms[nquadjetvars*icomb+3]->Fill(Hcand_1[icomb].DeltaR(Hcand_2[icomb]),weight_nom);
		h_quadjet_histograms[nquadjetvars*icomb+4]->Fill((Hcand_1[icomb]+Hcand_2[icomb]).M(),weight_nom);
		h_quadjet_histograms[nquadjetvars*icomb+5]->Fill((Hcand_1[icomb]+Hcand_2[icomb]).M(),weight_nom);
		h_quadjet_histograms[nquadjetvars*icomb+6]->Fill(Hcand_2[icomb].Pt()/Hcand_1[icomb].Pt(),weight_nom);
		h_quadjet_histograms[nquadjetvars*icomb+7]->Fill((Hcand_1[icomb]+Hcand_2[icomb]).M() - Hcand_1[icomb].M() - Hcand_2[icomb].M(),weight_nom);
		h_quadjet_histograms[nquadjetvars*icomb+8]->Fill(((Hcand_1[icomb]+Hcand_2[icomb]).M() - Hcand_1[icomb].M() - Hcand_2[icomb].M())/((Hcand_1[icomb]+Hcand_2[icomb]).M()),weight_nom);
		   
	   }
	   
	   if(scoremax_comb>=0 && scoremax_comb<=2){
	...
	// if(jet_idx_btag.size()>=4){
	   ...
	   if(Hmin_comb>=0 && Hmin_comb<=2) {
	
	   /*
	   TLorentzVector Hcand_1, Hcand_2;
	  
		// with minimum distance to mass condition //
		...
		*/
		...
		if(fabs(float(Hcand_1[Hmin_comb].M()) - SM_H_mass) < fabs(float(Hcand_2[Hmin_comb].M()) - SM_H_mass)) {     
			TLorentzVector p4;
			p4 = Hcand_1[Hmin_comb];
			Hcand_1[Hmin_comb] = Hcand_2[Hmin_comb];
			Hcand_2[Hmin_comb] = p4;
		}
		
		if(nJetAK4>=4){
						
			float unrol_mass = (getbinid(Hcand_1[Hmin_comb].M(),nmybins,mybins)>=0)?((Hcand_1[Hmin_comb]+Hcand_2[Hmin_comb]).M()+getbinid(Hcand_1[Hmin_comb].M(),nmybins,mybins)*mxbins[nmxbins-1]):-100;
			...
			float Hcand_mass = Hcand_2[Hmin_comb].M();
			...
			// tight SR (with mHcut)
			if(reg_pass[0] && Hcand_mass>=H_mass_min && Hcand_mass<H_mass_max) { reg_pass[4] = true; };  //5544 inclusive SR & mHcut
			if(reg_pass[1] && Hcand_mass>=H_mass_min && Hcand_mass<H_mass_max) { reg_pass[5] = true; };  //5544 SR & mHcut
			if(reg_pass[2] && Hcand_mass>=H_mass_min && Hcand_mass<H_mass_max) { reg_pass[6] = true; };  //5554 SR & mHcut
			if(reg_pass[3] && Hcand_mass>=H_mass_min && Hcand_mass<H_mass_max) { reg_pass[7] = true; };  //5555 SR & mHcut
			// loose SR, CR, VR (no mHcut)
			if(jet_btag_1>=3 && jet_btag_2>=3 && jet_btag_3>=3 && jet_btag_4>=2){ reg_pass[8] = true; };  //3332 SR
			if(jet_btag_1>=3 && jet_btag_2>=3 && jet_btag_3>=2 && jet_btag_4>=2){ reg_pass[9] = true; };  //3322 SR
			...
			if(reg_pass[8]  && Hcand_mass>=H_mass_min && Hcand_mass<H_mass_max) { reg_pass[12] = true; };  //3332 SR & mHcut
			if(reg_pass[9]  && Hcand_mass>=H_mass_min && Hcand_mass<H_mass_max) { reg_pass[13] = true; };  //3322 SR & mHcut
			if(reg_pass[10] && Hcand_mass>=H_mass_min && Hcand_mass<H_mass_max) { reg_pass[14] = true; };  //CR  & mHcut
			if(reg_pass[11] && Hcand_mass>=H_mass_min && Hcand_mass<H_mass_max) { reg_pass[15] = true; };  //VR  & mHcut
			// loose SR, CR, VR (inverted mHcut)
			if(reg_pass[8]  && !(Hcand_mass>=H_mass_min && Hcand_mass<H_mass_max)) { reg_pass[16] = true; };  //3332 SR & !mHcut
			if(reg_pass[9]  && !(Hcand_mass>=H_mass_min && Hcand_mass<H_mass_max)) { reg_pass[17] = true; };  //3322 SR & !mHcut
			if(reg_pass[10] && !(Hcand_mass>=H_mass_min && Hcand_mass<H_mass_max)) { reg_pass[18] = true; };  //CR & !mHcut
			if(reg_pass[11] && !(Hcand_mass>=H_mass_min && Hcand_mass<H_mass_max)) { reg_pass[19] = true; };  //VR & !mHcut
			
			for(int ireg=0; ireg<nregion; ireg++){
				
				if(reg_pass[ireg]){
					
					h_reg_MX[ireg]->Fill((Hcand_1[Hmin_comb]+Hcand_2[Hmin_comb]).M(),weight_nom);  
					h_reg_MY[ireg]->Fill((Hcand_1[Hmin_comb]).M(),weight_nom); 
					h_reg_MX_MY[ireg]->Fill(unrol_mass,weight_nom); 
					
					h_reg_dnn[ireg]->Fill(dnn_score,weight_nom);
					...
					for(int ijet=0; ijet<min(nJetAK4,4); ijet++){
						...
						h_AK4jets_regs[ireg][njetvars*ijet+4]->Fill(JetAK4_charge_kappa_0p3[ijet], weight_nom);
						h_AK4jets_regs[ireg][njetvars*ijet+5]->Fill(JetAK4_charge_kappa_0p6[ijet], weight_nom);
						h_AK4jets_regs[ireg][njetvars*ijet+6]->Fill(JetAK4_charge_kappa_1p0[ijet], weight_nom);
						if(year!="2024"){
							h_AK4jets_regs[ireg][njetvars*ijet+7]->Fill(JetAK4_btag_DeepFlavB[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+8]->Fill(JetAK4_btag_DeepFlavB_WP[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+9]->Fill(JetAK4_btag_DeepFlavQG[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+10]->Fill(JetAK4_btag_PNetB[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+11]->Fill(JetAK4_btag_PNetB_WP[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+12]->Fill(JetAK4_btag_PNetCvB[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+13]->Fill(JetAK4_btag_PNetCvL[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+14]->Fill(JetAK4_btag_PNetQG[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+15]->Fill(JetAK4_btag_RobustParTAK4B[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+16]->Fill(JetAK4_btag_RobustParTAK4B_WP[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+17]->Fill(JetAK4_btag_RobustParTAK4CvB[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+18]->Fill(JetAK4_btag_RobustParTAK4CvL[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+19]->Fill(JetAK4_btag_RobustParTAK4QG[ijet], weight_nom);
						}
						else{
							h_AK4jets_regs[ireg][njetvars*ijet+7]->Fill(JetAK4_btag_UParTAK4B[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+8]->Fill(JetAK4_btag_UParTAK4B_WP[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+9]->Fill(JetAK4_btag_UParTAK4CvB[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+10]->Fill(JetAK4_btag_UParTAK4CvL[ijet], weight_nom);
							h_AK4jets_regs[ireg][njetvars*ijet+11]->Fill(JetAK4_btag_UParTAK4QG[ijet], weight_nom);
						}
