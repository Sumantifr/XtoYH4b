...
////////////////////////////////////
// Useful small macros (by Mikko+Suman) //
////////////////////////////////////
...
void SetPlotStyle(TH1D* h, 
             int lstyle=kSolid, int lcolor=-1, int lwidth=1,
             int marker=kFullCircle, int mcolor = kBlack, int msize = 1,
             int fstyle=0, int fcolor=kYellow+1, float ftrans=0.5){
				 
h->SetLineStyle(lstyle);
h->SetLineColor(lcolor==-1 ? mcolor : lcolor);
h->SetLineWidth(lwidth);
h->SetMarkerStyle(marker);
h->SetMarkerColor(mcolor==-1 ? lcolor : mcolor);
h->SetMarkerSize(mcolor);
h->SetFillStyle(fstyle);
h->SetFillColor(fcolor);
h->SetFillColorAlpha(fcolor, ftrans);
				 
}


void SetAxisStyle(TH2D* h){

h->GetXaxis()->SetTitleSize(0.045);
h->GetXaxis()->SetLabelSize(0.045);
//h->GetXaxis()->CenterTitle();
h->GetXaxis()->SetTitleOffset(1.25);
h->GetYaxis()->SetNdivisions(604);
...
h->GetZaxis()->SetNdivisions(604);
h->GetZaxis()->SetNoExponent(0);

}

void SetAxisStyle(TH1D* h, bool isRatio=false, bool is2D=true){
...
void SetAxisStyle(TGraph *gr, int mcol=kRed, int msty= kFullCircle, float msize=1, int lcol=kRed, int lsty=kSolid, int lwidth=1){
...
gr->GetXaxis()->SetTitleOffset(5.75);
...
void tdrDraw(TH1D* h, string opt, int marker=kFullCircle, int mcolor = kBlack,
             int lstyle=kSolid, int lcolor=-1,
             int fstyle=1001, int fcolor=kYellow+1, float ftrans=0.5){
  h->SetMarkerStyle(marker);
  h->SetMarkerColor(mcolor);
  h->SetLineStyle(lstyle);
  h->SetLineColor(lcolor==-1 ? mcolor : lcolor);
...
TLegend *tdrLeg(double x1, double y1, double x2, double y2, int font, float textsize) {
...
TString lumi_13TeV_17BCDEF = "41.5 fb^{-1}";
TString lumi_13TeV_16BCDEFGH = "35.9 fb^{-1}";
TString lumi_13TeV_16BCDEF = "19.5 fb^{-1}";
TString lumi_13TeV_16FGH = "16.8 fb^{-1}";
TString lumi_13TeV_18ABCD = "59.7 fb^{-1}";
TString lumi_13TeV_Run2 = "137 fb^{-1}";
TString lumi_13TeV_Run3 = "300 fb^{-1}";
TString lumi_8TeV  = "19.7 fb^{-1}";
TString lumi_7TeV  = "5.1 fb^{-1}";

bool drawLogo      = false;

void CMS_lumi( TPad* pad, int iPeriod=3, int iPosX=10, bool SIM=false );
...
void 
CMS_lumi( TPad* pad, int iPeriod, int iPosX, bool SIM)
{            
...
*/
    ...
    {
      lumiText += lumi_13TeV_17BCDEF;
      lumiText += " (13 TeV)";
    }
   else if ( iPeriod==9 )
    {
      lumiText += lumi_13TeV_16BCDEFGH;
      lumiText += " (13 TeV)";
    }
   else if ( iPeriod==10 )
    {
      lumiText += lumi_13TeV_18ABCD;
      lumiText += " (13 TeV)";
    }
   else if ( iPeriod==11 )
    {
...
//      lumiText += lumi_13TeV_18ABCD;
  ...
  else if ( iPeriod==14 )
    {
      ...
      lumiText += lumi_13TeV_16FGH;
      lumiText += " (13 TeV)";
  }
  else if ( iPeriod==16 ){ 
      lumiText += lumi_13TeV_16BCDEF;
      lumiText += " (13 TeV)";
  }
  else if ( iPeriod==17 ){ 
      lumiText += lumi_13p6TeV_2022;
      lumiText += " (13.6 TeV)";
  }
  else if ( iPeriod==18 ){ 
      lumiText += lumi_13p6TeV_2022EE;
      lumiText += " (13.6 TeV)";
  }
  else if ( iPeriod==19 ){ 
      lumiText += lumi_13p6TeV_2023;
      lumiText += " (13.6 TeV)";
  }
  else if ( iPeriod==20 ){ 
...
//	      if (extraText2!="") // For Simulation Preliminary
		...
		latex.DrawLatex(posX_, posY_-relExtraDY*cmsTextSize*t
				- relExtraDY*extraTextSize*t, extraText3);
      ...
      if( iPosX==0) 
	{
      ...
      latex.SetTextAlign(align_);
      if(SIM){
      latex.DrawLatex(posX_, posY_, extraText3);
      }else{
...
void write_process(bool writeprocess, TString pname, float pos_x, float pos_y)
{
...
latex.SetTextFont(42);
latex.SetTextAlign(31); 
latex.SetTextSize(0.075*lumiTextSize);  
if(writeprocess){   
	latex.DrawLatex(pos_x,pos_y,pname);
}
}
...
TCanvas* tdrCanvas(const char* canvName, TH1D *h,
		   ...
		   bool p_write = false, string p_name = "",
		   bool SIM = false,
  ...
  CMS_lumi( canv, iPeriod, iPos, SIM );
  write_process(p_write,p_name,0.85,0.85);
...
TCanvas* tdrDiCanvas(const char* canvName, TH1D *hup, TH1D *hdw,
 ...
 //cout<<"Width "<<W<<" Height "<<H<<endl;
  ...
  // writing the lumi information and the CMS "logo"
  CMS_lumi( (TCanvas*)gPad, iPeriod, iPos, SIM );
  ...
  write_process(p_write,p_name,0.9,0.9);
...
TCanvas* tdrmyDiCanvas(const char* canvName, TH1D *hup, TH1D *hdw, float iScale,
                   int iPeriod = 7, int iPos = 11, bool SIM = false) {
