...
float* TriggerSF_btag(TFile *file_trigger_btag, vector<AK4Jet> inputjets, string year){
	...
	else if (year=="2023"||year=="2023BPiX") { x_input = 0.5*(jets[0].btag_PNetB+jets[1].btag_PNetB);	}
	const double* xArray = grjet->GetX();
...
//   fileout->SetCompressionLevel(4); 
   ...
   Tout->Branch("H_cand_pt", &YH_cand_pt, "H_cand_pt/F");
   ...
   Tout->Branch("angle_theta_H", &angles_XYH[0], "angles_XYH[0]/F");
   ...
   */
   ...
   for (int ij=0; ij<nentries; ij++) {
    ...
    for(unsigned ijet=0; ijet<Jets.size(); ijet++){
		...
		if(ijet<4) { bp4s.push_back(Jets[ijet].p4); }
   ...
   if(bp4s.size()==4)
	   ...
	   if(index_H_b2>=0) { bp4s.remove(bp4s.begin()+index_H_b2); }
	   if(index_H_b1>=0) { bp4s.remove(bp4s.begin()+index_H_b1); }
	   ...
	   angles_XYH = get_Xto4b_angles(H_b1,H_b2,Y_b1,Y_b2);
   ...
   */
	...
	if(!Flag_pass_baseline){ 
		...
		JetCombination_Btag_ParT_index = JetCombination_Btag_PNet_index = 0;
